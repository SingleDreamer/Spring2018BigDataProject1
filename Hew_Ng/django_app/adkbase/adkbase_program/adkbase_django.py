...
def run_servers():
    ...
    mongod = ["./adkbase/adkbase_program/mongodb-osx-x86_64-3.6.3/bin/mongod", "-dbpath", "adkbase/adkbase_program/data/mongo"]
    redisserver = ["./adkbase/adkbase_program/redis-stable/src/redis-server", "adkbase/adkbase_program/redis-stable/redis.conf"]
...
def reset_redis():
    if (not is_redis_available()):
        ...
        return
    redis_db.flushdb()
    ppi_file = csv.reader(open("../adkbase_program/raw_data/PPI.csv"))
    for line in ppi_file:
        redis_db.rpush(line[0], line[1])

    redis_two.flushdb()
    entrezuniprot_file = csv.reader(open("../adkbase_program/raw_data/entrez_ids_uniprot.txt"), delimiter = '\t')
...
def reset_sql():
    ...
    with open('../adkbase_program/raw_data/patients.csv', 'rb') as csvfile:
    ...
    with open('../adkbase_program/raw_data/entrez_ids_genesymbol.csv', 'rb') as csvfile:
        reader = csv.reader(csvfile)
        next(reader)
        for row in reader:
...
def reset_mongo():
    ...
    csvfile = open('../adkbase_program/raw_data/ROSMAP_RNASeq_entrez.csv', 'rb')
    reader = csv.reader(csvfile)
