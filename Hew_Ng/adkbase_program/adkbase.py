...
def reset_redis():
    ...
    redis_two.flushdb()
    entrezuniprot_file = csv.reader(open("raw_data/entrez_ids_uniprot.txt"), delimiter = '\t')
...
def get_c1(input_entrez_id):
    ...
    if (not is_redis_available()):
        ...
        try:
            ...
            return output
        output+=  "interactor A\tinteractor B"
        for b in redis_db.lrange(input_entrez_id, 0,-1):
            output+= "" + str(input_entrez_id) + "\t\t" +  b + "\n"
        return output
...
def get_c3(input_entrez_id):
    ...
    if ans is None:
        ...
        return output
    output += ("\n")
    output += ("Gene ID: " + str(ans[0]) + "\n")
    output += ("Gene Symbol: " + ans[1] + "\n")
    output += ("Gene Name: " + ans[2] + "\n")
